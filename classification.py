...
class CustomModelforSequenceClassification(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        if self.type == "full":            
            ...
            prefix_length = self.prefix.size(0)
            expanded_prefix = self.prefix.unsqueeze(0).repeat(batch_size, 1, 1)
            ...
            input_embed = self.model.embeddings.word_embeddings(input_ids).to('cuda')
            ...
            inputs_embeds = torch.cat([expanded_prefix, input_embed], dim=1).to('cuda')
            ...
            attention_mask = torch.cat([torch.ones(batch_size, prefix_length).to('cuda'), attention_mask], dim=1)
...
def train(mymodel, num_epochs, train_dataloader, validation_dataloader, test_dataloder, device, lr, model_name):
    ...
    if mymodel.type == "head":
        # implement the optimizer for head-tuned model
        ...
        all_params = prefix_params + classifier_params
        # your code ends here
        
        # group the parameters together
        custom_optimizer = torch.optim.AdamW(all_params, lr=lr)
    ...
    for epoch in range(num_epochs):
        ...
        for index, batch in tqdm(enumerate(train_dataloader)):
            ...
            with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA], profile_memory=True) as prof:
                # run forward and name the output as `output`
                output = mymodel(input_ids=input_ids, attention_mask=attention_mask)
                logits = output['logits']

                # compute the loss using the loss function
                loss_val = loss(logits, labels)  # Compute loss

            # Print forward pass memory usage
            print("Forward Pass Memory Usage for " + mymodel.type + " Tuning:")
            ...
            if mymodel.type == "full" or mymodel.type == "auto":
                # Start profiling for backward pass
                with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA], profile_memory=True) as prof:
                    loss_val.backward()  # Compute gradients
                    optimizer.step()  # Update weights
                    lr_scheduler.step()  # Update learning rate
            else:
                # Similar profiling block for custom_optimizer.step() if mymodel.type is not "full" or "auto"
                with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA], profile_memory=True) as prof:
                    loss_val.backward()  # Compute gradients
                    custom_optimizer.step()  # Update weights
                    lr_scheduler.step()  # Update learning rate

            # Print backward pass memory usage
            print("Backward Pass Memory Usage for " + mymodel.type + " Tuning:")
            print(prof.key_averages().table(sort_by="cuda_memory_usage", row_limit=10))
