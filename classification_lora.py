...
def print_learnable_parameters(model):
    ...
    print(f"Number of learnable parameters: {params}")
...
def evaluate_model(model, dataloader, device):
    ...
    for batch in dataloader:
        # Implemented the evaluation function get the input_ids, attention_mask from the batch and put them on the device
        input_ids = batch['input_ids'].to(device)
        attention_mask = batch['attention_mask'].to(device)
        
        # forward pass
        with torch.no_grad(): output = model(input_ids=input_ids, attention_mask=attention_mask)
...
def train(mymodel, num_epochs, train_dataloader, validation_dataloader, test_dataloder, device, lr, small_subset=False):
    ...
    # LoRA: Instead of using mymodel.parameters(), we ensure all parameters that require gradients are included.
    optimizer_parameters = [p for p in mymodel.parameters() if p.requires_grad]
...
def pre_process(model_name, batch_size, device, small_subset):
    ...
    # Wrap the model with LoRA using get_peft_model
    ...
    print_learnable_parameters(lora_model)

    return lora_model, train_dataloader, validation_dataloader, test_dataloader
